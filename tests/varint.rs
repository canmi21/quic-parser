...
use quic_parser::{Error, read_varint_at};

#[test]
fn one_byte_zero() {
	let mut cursor = 0;
	let val = read_varint_at(&[0x00], &mut cursor).unwrap();
	assert_eq!(val, 0);
	assert_eq!(cursor, 1);
}
...
fn one_byte_max() {
	let mut cursor = 0;
	let val = read_varint_at(&[0x3f], &mut cursor).unwrap();
	assert_eq!(val, 63);
	assert_eq!(cursor, 1);
}
...
fn two_byte_min() {
	let mut cursor = 0;
	let val = read_varint_at(&[0x40, 0x00], &mut cursor).unwrap();
	assert_eq!(val, 0);
	assert_eq!(cursor, 2);
}
...
fn two_byte_example() {
	...
	let mut cursor = 0;
	let val = read_varint_at(&[0x7b, 0xbd], &mut cursor).unwrap();
	assert_eq!(val, 15293);
	assert_eq!(cursor, 2);
}
...
fn four_byte_example() {
	...
	let mut cursor = 0;
	let val = read_varint_at(&[0x9d, 0x7f, 0x3e, 0x7d], &mut cursor).unwrap();
...
fn eight_byte_example() {
	...
	let mut cursor = 0;
	let val = read_varint_at(&[0xc2, 0x19, 0x7c, 0x5e, 0xff, 0x14, 0xe8, 0x8c], &mut cursor).unwrap();
...
fn empty_buffer_fails() {
	assert!(matches!(read_varint_at(&[], &mut 0), Err(Error::BufferTooShort { need: 1, have: 0 })));
}

#[test]
fn truncated_two_byte() {
	assert!(matches!(read_varint_at(&[0x40], &mut 0), Err(Error::BufferTooShort { need: 2, have: 1 })));
}

#[test]
fn truncated_four_byte() {
	assert!(matches!(
		read_varint_at(&[0x80, 0x00], &mut 0),
...
fn truncated_eight_byte() {
	assert!(matches!(
		read_varint_at(&[0xc0, 0x00, 0x00], &mut 0),
...
fn extra_bytes_ignored() {
	...
	let mut cursor = 0;
	let val = read_varint_at(&buf, &mut cursor).unwrap();
	assert_eq!(val, 37);
	assert_eq!(cursor, 1);
}
...
fn boundary_one_to_two_byte() {
	...
	cursor = 0;
	let val = read_varint_at(&[0x40, 0x40], &mut cursor).unwrap();
	assert_eq!(val, 64);
	assert_eq!(cursor, 2);
}
...
fn two_byte_max() {
	...
	let mut cursor = 0;
	let val = read_varint_at(&[0x7f, 0xff], &mut cursor).unwrap();
	assert_eq!(val, 16383);
	assert_eq!(cursor, 2);
}
...
fn boundary_two_to_four_byte() {
	...
	let mut cursor = 0;
	let val = read_varint_at(&[0x80, 0x00, 0x40, 0x00], &mut cursor).unwrap();
	assert_eq!(val, 16384);
	assert_eq!(cursor, 4);
}
...
fn four_byte_max() {
	...
	let mut cursor = 0;
	let val = read_varint_at(&[0xbf, 0xff, 0xff, 0xff], &mut cursor).unwrap();
...
fn boundary_four_to_eight_byte() {
	...
	let mut cursor = 0;
	let val = read_varint_at(&[0xc0, 0x00, 0x00, 0x00, 0x40, 0x00, 0x00, 0x00], &mut cursor).unwrap();
...
fn eight_byte_max() {
	...
	let mut cursor = 0;
	let val = read_varint_at(&[0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff], &mut cursor).unwrap();
...
fn truncated_four_byte_at_two() {
	...
	assert!(matches!(
		read_varint_at(&[0x80, 0x01], &mut 0),
		Err(Error::BufferTooShort { need: 4, have: 2 })
	));
}
...
fn truncated_four_byte_at_three() {
	...
	assert!(matches!(
		read_varint_at(&[0x80, 0x01, 0x02], &mut 0),
...
fn truncated_eight_byte_at_four() {
	...
	assert!(matches!(
		read_varint_at(&[0xc0, 0x01, 0x02, 0x03], &mut 0),
...
fn truncated_eight_byte_at_seven() {
	...
	assert!(matches!(
		read_varint_at(&[0xc0, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06], &mut 0),
